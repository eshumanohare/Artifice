...
export async function GET(request: NextRequest) {
  try {
    console.log(`🔄 Cron job: Generating fresh orders and adding to data store...`);
    
    // Generate 1-2 new orders with current block numbers
    const currentBlock = 78149000 + Math.floor(Math.random() * 1000);
    const numNewOrders = Math.floor(Math.random() * 2) + 1; // 1-2 new orders
    const freshOrders = [];
    ...
    console.log(`📊 Cron: Generated ${freshOrders.length} new orders with block ${currentBlock}`);
    
    // Add new orders to existing data store (not replace)
    updateOrders(freshOrders, currentBlock);
    
    console.log(`✅ Cron: Successfully added ${freshOrders.length} new orders to data store`);
